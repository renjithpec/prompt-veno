...
export function SelectTrigger({ className, children }: SelectPrimitive.SelectTriggerProps) {
  return (
    <SelectPrimitive.Trigger className={cn("tap flex w-full items-center justify-between rounded-card border border-white/10 bg-white/5 px-3 text-sm text-white outline-none focus:border-accent/60", className)}>
      {children}
...
export function SelectContent({ className, children }: SelectPrimitive.SelectContentProps) {
  return (
    <SelectPrimitive.Portal>
      <SelectPrimitive.Content className={cn("z-50 min-w-40 overflow-hidden rounded-card border border-white/10 bg-[#101010] p-1 text-white shadow-xl", className)}>
...
export function SelectItem({ className, children, value }: SelectPrimitive.SelectItemProps) {
  return (
    <SelectPrimitive.Item value={value} className={cn("relative flex min-h-10 cursor-pointer select-none items-center rounded-md px-8 text-sm outline-none hover:bg-white/10 data-[highlighted]:bg-white/10", className)}>
